...
-- book_coupon_template 테이블 생성
DROP TABLE IF EXISTS `book_coupon_template`;
...
-- category_coupon_template 테이블 생성
DROP TABLE IF EXISTS `category_coupon_template`;
...
-- wrapping_paper 테이블의 외래 키 추가
-- 별도의 외래 키가 없는 경우는 생략

-- delivery_fee 테이블의 외래 키 추가
-- 별도의 외래 키가 없는 경우는 생략

-- orders_status 테이블의 외래 키 추가
-- 별도의 외래 키가 없는 경우는 생략

-- point_policy 테이블의 외래 키 추가
-- 별도의 외래 키가 없는 경우는 생략

-- point_history 테이블의 외래 키 추가
ALTER TABLE `point_history`
    ADD CONSTRAINT `fk_point_history_orders_id` FOREIGN KEY (`orders_id`) REFERENCES `orders`(`id`),
...
-- coupon 테이블의 외래 키 추가
ALTER TABLE `coupon`
    ADD CONSTRAINT `fk_coupon_coupon_template_id` FOREIGN KEY (`coupon_template_id`) REFERENCES `coupon_template`(`id`);
...
-- review 테이블의 외래 키 추가
ALTER TABLE `review`
    ADD CONSTRAINT `fk_review_book_id` FOREIGN KEY (`book_id`) REFERENCES `book`(`id`),
    ADD CONSTRAINT `fk_review_orders_id` FOREIGN KEY (`orders_id`) REFERENCES `orders`(`id`);
...
-- category_coupon_template 테이블의 외래 키 추가
ALTER TABLE `category_coupon_template`
    ADD CONSTRAINT `fk_category_coupon_template_coupon_template_id` FOREIGN KEY (`coupon_template_id`) REFERENCES `coupon_template`(`id`);

-- book_coupon_template 테이블의 외래 키 추가
ALTER TABLE `book_coupon_template`
    ADD CONSTRAINT `fk_book_coupon_template_coupon_template_id` FOREIGN KEY (`coupon_template_id`) REFERENCES `coupon_template`(`id`);